...
3. Create scripts/test.sh that runs cas24 on a trivial .s input and compares
   output bytes against the cross-assembler (as24).
4. Define static allocation strategy: code buffer, symbol table arrays,
   forward ref arrays, input line buffer, token buffer. Document size limits.
